...
func NewAnonymiser(s Store) *anonymiser {
	return &anonymiser{
		store: s,
	}
}
...
func (a *anonymiser) AnonymiseRows(table string, rowChan chan<- []*Cell, endChan chan<- bool) error {
...
func anonymise(column, replacement string) (*Cell, error) {
	literal := strings.TrimPrefix(replacement, literalPrefix)
	scanner := new(scanner)

	if scanner.Scan(literal); len(literal) != len(replacement) {
		return &Cell{Column: column, Type: scanner.Detected, Value: literal}, nil
	}
	...
	if replacement != "" && len(literal) == len(replacement) {
		...
		return &Cell{Column: column, Type: scanner.Detected, Value: value}, nil
	}
